...
本ドキュメントは、`shifree`（旧 shift-scheduler-app）の全ソースコード、データベーススキーマ、APIエンドポイント、サービス層ロジック、およびフロントエンド資産を網羅した詳細な技術辞書です。本システムは Flask を用いたバックエンドと、Vanilla JS を用いたフロントエンドによって構成され、Vercel (Serverless) 環境での動作を前提として設計されています。
